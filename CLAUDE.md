...
- **Map controls**: LMB = select / target. RMB-tap = deselect / cancel-targeting. RMB-drag (past ~5 px) = pan camera (cursor flips to `grabbing`). Wheel = zoom (cursor-centred when nothing's selected). Arrow keys / WASD = pan camera while held (skipped when a form input has focus). Targeting cursor (crosshair) wins over the pan cursor.
